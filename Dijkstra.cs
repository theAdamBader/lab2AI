...
public class Dijkstra : Pathfinder
{
    ...
    public List<Node> FindPath(Node start, Node goal)
    {
        ...
        Console.WriteLine("\nUsing Dijkstra to look for a path from " + start + " to " + goal);
    ...
    // Dijkstra search: returns true if path found
        ...
        while (frontier.Count != 0)
        {
            ...
            foreach (Node next in neighbours)
            {
                ...
                if (!visitedBefore || nextCost < cost[next])
                {
                    ...
                    // Removing the Distance variable from there was a must for A* algorithm as it calculates the path closet to the goal while Dijkstra searches all the path so the Distance variable is not needed
    ...
    // This function is used in the Example.cs to draw the edges in the AddEdges function
    // The straight line distance between nodes
